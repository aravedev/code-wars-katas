...
function DNAStrand(dna) {
  ...
  let result = dna.split("").map((e) => obj[e]);
...
function removeSmallest(numbers) {
  ...
  let min = Math.min(...copyNumbers);
  const index = copyNumbers.indexOf(min);
  let result = copyNumbers.splice(index, 1);
  return copyNumbers;
}
...
function dirReduc(arr) {
  ...
  for (let i = copy.length; i >= 0; i--) {
    if (
      (copy[i] == "NORTH" && copy[i - 1] == "SOUTH") ||
      (copy[i] == "SOUTH" && copy[i - 1] == "NORTH")
    ) {
      copy.splice(i - 1, 2);
    } else if (
      (copy[i] == "EAST" && copy[i - 1] == "WEST") ||
      (copy[i] == "WEST" && copy[i - 1] == "EAST")
    ) {
      copy.splice(i - 1, 2);
    }
  }
...
const str = ["NORTH", "SOUTH", "EAST", "WEST", "EAST", "WEST"];
console.log(dirReduc(str));
...
function fibo(position) {
  let x = 1.618034 ** position - (1 - 1.618034) ** position;
  x = x / Math.sqrt(5);
...
//My solution:
  ...
  if (result === prod) {
    ...
    return productFib(prod, n + 1);
  }
}

function fibo(position) {
  let x = 1.61803398875 ** position - (1 - 1.61803398875) ** position;
  x = x / Math.sqrt(5);
...
function domainName(url) {
  let str = url;
  let com = str.indexOf(".com");
  let result = "";
  str = str.substr(0, com);

  let dash = str.indexOf("//");
  let www = str.indexOf("www.");

  if (dash !== -1) {
    result = str.substr(dash + 2);
    return result;
  } else if (www !== -1) {
  }
...
// My solution with help from the forum
...
function removeUrlAnchor(url) {
  if (url.includes("#")) {
    ...
    return url.substring(0, remove);
  } else {
    return url;
  }
}

const str = "www.codewars.com#about";
...
function stockList(listOfArt, listOfCat) {
  if (listOfArt.length === 0 || listOfCat.length == 0) {
    ...
    listOfArt.forEach((book) => {
      ...
      if (listOfCat.includes(code)) {
        obj[code] = obj[code] + Number(temp[1]);
      }
    });

    let tempArr = Object.entries(obj);
    tempArr = tempArr.map((book) => book.join(" : "));
    tempArr = tempArr.map((book, index) => `(${tempArr[index]})`);
